...
func (f *fnCtx) fstr(e *frontend.FStr) (ast.Expr, error) {
	...
	for _, p := range e.Parts {
		...
		case *frontend.FText:
			...
			parts = append(parts, callExpr(f.e.obj("NewStr"), strLit(p.Text)))
	...
	return callExpr(sel("runtime", "JoinStrs"), parts...), nil
}
...
func (f *fnCtx) fInterp(p *frontend.FInterp) ([]ast.Expr, error) {
	...
	if p.HasSpec {
		...
		// rules apply to the converted text, matching CPython.
		tmp := f.tmpVar()
		f.fallible(tmp, f.e.obj("Format"), v, strLit(p.Spec))
