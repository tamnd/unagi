...
// character. The canonical combining class, the raw decomposition strings, and
// the decomposition and composition mappings that back normalize / is_normalized
// are pinned from the CPython 3.14.6 UCD in unicodedata_tables.go (generated by
// gen_unicodedata_tables.py), so those four agree with CPython character for
// character too. The remaining UCD properties (bidi class, the name database)
// are not part of the Go standard library and are not bundled here, so those
// functions carry best-effort implementations documented on each. unidata_version
// reports the UCD version Go's `unicode` package actually ships rather than
// claiming CPython's, so a caller that inspects the category path is not misled.
