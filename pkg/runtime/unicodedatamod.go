...
// the decimal/digit/numeric values, and the Bidi_Mirrored flag are pinned from
// the CPython 3.14.6 UCD in unicodedata_tables.go (generated by
// gen_unicodedata_tables.py), so those agree with CPython character for character
// too. The remaining UCD properties (bidi class, the name database) are not part
// of the Go standard library and are not bundled here, so those functions carry
// best-effort implementations documented on each. unidata_version reports the
