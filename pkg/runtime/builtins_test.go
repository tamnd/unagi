...
func TestSum(t *testing.T) {
	tests := []struct {
		...
		{"str-start-int-elts", objs(newList(i(1), i(2)), s("x")), "",
			"TypeError: sum() can't sum strings [use ''.join(seq) instead]"},
		{"bytes-start", objs(newList(objects.NewBytes([]byte("a"))), objects.NewBytes(nil)), "",
			"TypeError: sum() can't sum bytes [use b''.join(seq) instead]"},
		{"bytes-start-empty-iter", objs(newList(), objects.NewBytes(nil)), "",
			"TypeError: sum() can't sum bytes [use b''.join(seq) instead]"},
		{"bytearray-start", objs(newList(), objects.NewByteArray(nil)), "",
