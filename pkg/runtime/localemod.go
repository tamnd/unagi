...
// the C locale this is a plain code-point comparison, returning -1, 0 or 1.
...
func localeSetlocale(pos []objects.Object, kwNames []string, kwVals []objects.Object) (objects.Object, error) {
	...
	if _, ok := objects.AsInt(category); !ok {
	...
	if localeErrorClass != nil {
		if inst, err := objects.Call(localeErrorClass, []objects.Object{objects.NewStr("unsupported locale setting")}); err == nil {
			if e, ok := inst.(error); ok {
				return nil, e
			}
		}
	}
	return nil, objects.Raise(objects.ValueError, "unsupported locale setting")
}
