...
func TestLexTokens(t *testing.T) {
	tests := []struct {
		...
		{"raw fstring keeps backslashes", `rf"\n{x}\t"`, `fstart:rf" fmid:"\\n" f{ x f} fmid:"\\t" fend:" NL EOF`},
		{"fstring raw alt order", `fr"C:\{x}"`, `fstart:fr" fmid:"C:\\" f{ x f} fend:" NL EOF`},
		{"raw fstring spec keeps backslash", `rf"{x:\>4}"`, `fstart:rf" f{ x fmid:"\\>4" f} fend:" NL EOF`},
...
func TestLexErrors(t *testing.T) {
	tests := []struct {
		...
		{"junk after imaginary", "x = 3jabc", "invalid imaginary literal"},
		{"imaginary radix", "x = 0x1j", "invalid hexadecimal literal"},
		...
		{"tstring with bytes prefix", `bt"x"`, "is not a valid string prefix"},
		{"tstring with f prefix", `ft"x"`, "is not a valid string prefix"},
